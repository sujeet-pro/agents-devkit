...
Do not duplicate the canonical content here — Cursor reads this pointer, follows the link,
and uses the version-pinned references inside `node_modules/`. That keeps every invocation
in lockstep with the installed `@pagesmith/docs` and `diagramkit` versions.
