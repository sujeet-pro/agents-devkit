...
- Parallel fan-out: multiple code-reviewer agents analyze different aspects simultaneously (already implemented in /pr-review)
...
**When to act**: After `/pr-review` has been used manually for several weeks and the team is confident in its accuracy. Automated posting requires high precision to avoid noise.
...
- The function runs the `/pr-review` workflow and posts findings as review comments
