...
1. For each fixed comment, post a reply explaining the fix:
   ...
   - Google Docs: do **not** post replies via MCP (unreliable). Instead, produce a markdown file at `.temp/adk-docs-write/<doc-title>-fix-replies.md` listing each reply with its target comment and content. Present the file path and ask the user to add replies manually.
...
- `/adk-code-review-pr` for comment-only review of documents
