# Constitution for `adk-audit-repo`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/audit-repo-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-audit-repo`)
...
## Communication rules
- Lead with the status banner from `references/audit-repo-persona.md`: `AUDIT-DRAFT  |  AUDIT-FINAL <n> Blocker / <n> Critical / ...`.
...
## Research discipline
- Sources are consulted in the order specified by `references/audit-repo-research-protocol.md`. Higher-ranked sources win conflicts.
- Stop when the protocol's stop condition is met. Do not keep researching past diminishing returns.

## Multi-repo discipline
- Treat each passed repo independently per `references/audit-repo-multi-repo.md`. Tag every finding with the repo of origin.
