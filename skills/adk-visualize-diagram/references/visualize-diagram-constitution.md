...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/visualize-diagram-artifact-format.md` for the path matrix.
...
## Communication rules
- Lead with the status banner from `references/visualize-diagram-persona.md`: `DIAGRAM-RENDERED <path-light.svg + path-dark.svg>  |  RENDER-FAILED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/visualize-diagram-research-protocol.md`. Higher-ranked sources win conflicts.
