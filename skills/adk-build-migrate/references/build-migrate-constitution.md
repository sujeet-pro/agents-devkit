# Constitution for `adk-build-migrate`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/build-migrate-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-build-migrate`)
...
## Communication rules
- Lead with the status banner from `references/build-migrate-persona.md`: `MIGRATION-IN-PROGRESS step <i>/<n>  |  MIGRATION-DONE  |  ROLLED-BACK at step <i>`.
...
## Research discipline
- Sources are consulted in the order specified by `references/build-migrate-research-protocol.md`. Higher-ranked sources win conflicts.
