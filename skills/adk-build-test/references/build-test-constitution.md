# Constitution for `adk-build-test`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/build-test-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-build-test`)
...
## Communication rules
- Lead with the status banner from `references/build-test-persona.md`: `TESTS-ADDED <n>  |  TESTS-PASSING <n>/<m>  |  COVERAGE-DELTA +<x>%  |  BLOCKED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/build-test-research-protocol.md`. Higher-ranked sources win conflicts.
