...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/build-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-build`)
...
## Communication rules
- Lead with the status banner from `references/build-persona.md`: `ROUTED <build-task>  |  REJECTED-MIXED-INTENT (please split)`.
