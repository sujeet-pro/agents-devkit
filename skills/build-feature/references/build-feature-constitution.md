# Constitution for `adk-build-feature`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/build-feature-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-build-feature`)
...
## Communication rules
- Lead with the status banner from `references/build-feature-persona.md`: `DONE  |  DONE_WITH_CONCERNS  |  NEEDS_CONTEXT  |  BLOCKED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/build-feature-research-protocol.md`. Higher-ranked sources win conflicts.
