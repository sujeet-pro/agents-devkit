# Constitution for `adk-frontend-feature`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/frontend-feature-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-frontend-feature`)
...
## Communication rules
- Lead with the status banner from `references/frontend-feature-persona.md`: `FE-FEATURE-DONE  |  FE-FEATURE-WITH-CONCERNS  |  BLOCKED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/frontend-feature-research-protocol.md`. Higher-ranked sources win conflicts.
