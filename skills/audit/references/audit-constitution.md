...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/audit-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-audit`)
...
## Communication rules
- Lead with the status banner from `references/audit-persona.md`: `ROUTED <audit-task>`.
