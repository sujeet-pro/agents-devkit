...
## Communication rules
- Lead with the status banner from `references/persona.md`: `ANSWERED <confidence%>  |  PARTIAL (open questions)  |  CONTRADICTORY (sources disagree)`.
...
- Treat each passed repo independently per `references/multi-repo.md`. Tag every finding with the repo of origin.
