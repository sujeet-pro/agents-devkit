# Constitution for `adk-visualize`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/visualize-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-visualize`)
...
## Communication rules
- Lead with the status banner from `references/visualize-persona.md`: `ROUTED <visualize-task>`.
