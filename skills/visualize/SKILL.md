...
- Architectural design write-up (text-heavy) -> `@adk:plan-design` (a.k.a. `adk-plan-design`) (which may then call this category for one diagram)
...
| --- | --- | --- |
| `@adk:visualize-diagram` (a.k.a. `adk-visualize-diagram`) | Sequence, flow, state, ER, architecture, dependency, class, deployment, or any structural picture | Numeric / categorical data plot - use chart |
| `@adk:visualize-chart` (a.k.a. `adk-visualize-chart`) | Bar, line, pie, scatter, histogram, stacked area, heatmap from numeric or categorical data | Structural / relationship picture - use diagram |
