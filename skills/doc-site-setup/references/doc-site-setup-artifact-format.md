...
The deliverable from `adk-doc-site-setup` is the artifact described below. Working notes (plans, drafts, raw evidence) ALWAYS live under `.temp/` in the host repo. The artifact itself lives wherever this skill says it lives — sometimes under `.temp/`, sometimes in the repo proper, sometimes as a remote object (PR comment, Confluence page).

## Artifact type
`doc-site-bootstrap`

## Format
Files in the consumer repo (config + scaffold + scripts + workflow) + prj-doc-site-* skills mirrored into harness folders.

## Path / Location
Consumer repo: pagesmith.config.json5, diagramkit.config.json5, docs/, .github/workflows/gh-pages.yml, .agents/skills/prj-doc-site-*/, harness mirrors. Setup log in .temp/notes/doc-site-setup-<date>.md.
