...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/review-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-review`)
...
## Communication rules
- Lead with the status banner from `references/review-persona.md`: `ROUTED <review-task>`.
