...
## Platform Connector Skills (Fallback)

When the MCP is not available or doesn't support the required operation, invoke the platform connector skill:

- **GitHub**: `/adk:github` — uses `gh` CLI. Validates install and auth automatically.
- **Bitbucket**: `/adk:bitbucket` — uses REST API via `curl` + `BITBUCKET_USERNAME`/`BITBUCKET_TOKEN` from `~/.zshenv`.
- **Confluence**: `/adk:confluence` — uses REST API via `curl` + `CONFLUENCE_*` from `~/.zshenv`.
- **Jira**: `/adk:jira` — uses REST API via `curl` + `JIRA_*` from `~/.zshenv`.

The connector skills also detect MCP connectors internally and prefer them when available, falling back to direct API calls for unsupported operations.
