# Constitution for `adk-review-feedback`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/review-feedback-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-review-feedback`)
...
## Communication rules
- Lead with the status banner from `references/review-feedback-persona.md`: `RESOLVED <n>/<m>  |  PUSHED-BACK <n>  |  AWAITING-USER on <n>`.
...
## Research discipline
- Sources are consulted in the order specified by `references/review-feedback-research-protocol.md`. Higher-ranked sources win conflicts.
