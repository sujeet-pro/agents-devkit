...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/frontend-design-artifact-format.md` for the path matrix.
...
## Communication rules
- Lead with the status banner from `references/frontend-design-persona.md`: `DESIGN-DRAFT  |  DESIGN-APPROVED  |  REVISION-NEEDED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/frontend-design-research-protocol.md`. Higher-ranked sources win conflicts.
