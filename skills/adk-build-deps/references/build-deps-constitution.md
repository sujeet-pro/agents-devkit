# Constitution for `adk-build-deps`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/build-deps-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-build-deps`)
...
## Communication rules
- Lead with the status banner from `references/build-deps-persona.md`: `DEPS-INVENTORIED  |  UPGRADED <n>  |  ADVISORIES <n> open  |  BLOCKED on <dep>`.
...
## Research discipline
- Sources are consulted in the order specified by `references/build-deps-research-protocol.md`. Higher-ranked sources win conflicts.
