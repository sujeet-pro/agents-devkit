# plan-brainstorm Validator

The validator gate `adk-plan-brainstorm` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/plan-brainstorm-<slug>-validator.md`.
...
| --- | --- | --- |
| Question clear | User's actual question / ambiguity is restated in one sentence | BLOCKER if ambiguous — clarify |
| Stakes captured | Blast radius, reversibility, deadline, audience documented | BLOCKER without — cannot pick safely |
...
## Phase 3: Pre-handoff validation (decision quality)

Run after the brainstorm converges; verify the chosen direction is well-defended.

| Check | Pass criteria | Evidence captured |
| --- | --- | --- |
| ≥ 2 viable options surfaced | Not one option dressed as three; each has Pros / Cons / Best when / Blast radius / Reversibility | Option count + per-option fields |
| Trade-offs explicit | Why this option vs the others, in one sentence per dimension | Trade-off matrix |
| Default marked | Exactly one option marked `(default)` with rationale | Default + rationale |
| Next-step routing | The brainstorm names the next ADK skill (spec / design / roadmap / build / write-docs) | Next-step skill name |
...
| --- | --- | --- |
| Brainstorm artifact written | `.temp/drafts/brainstorm-<slug>.md` exists with options, trade-offs, decision, next step | File path + size |
...
The validator writes its check log to `.temp/notes/plan-brainstorm-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/plan-brainstorm-<slug>.md
```
