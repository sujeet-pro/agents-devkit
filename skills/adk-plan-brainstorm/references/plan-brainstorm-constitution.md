# Constitution for `adk-plan-brainstorm`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/plan-brainstorm-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-plan-brainstorm`)
...
## Communication rules
- Lead with the status banner from `references/plan-brainstorm-persona.md`: `FINALIZED <direction> → <next-skill>  |  ASK-USER (questions still open)  |  RESEARCH-BLOCKED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/plan-brainstorm-research-protocol.md`. Higher-ranked sources win conflicts.
