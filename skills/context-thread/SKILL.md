---
name: context-thread
description: "Use when you need persistent, named context threads for ongoing work streams that survive across sessions"
user_invocable: true
arguments:
  - name: action
    description: "Action: create, update, load, list, archive (default: list)"
    required: false
  - name: name
    description: "Thread name (for create/update/load/archive)"
    required: false
  - name: note
    description: "Note to append (for action=update)"
    required: false
---
...
`zsh scripts/check-skill-deps.zsh context-thread`
...
1. **Determine action.** Default to `list` if no action is specified.
2. **Validate inputs.** Check that required arguments are present for the chosen action.
3. **Execute action.** Follow the steps for the chosen action above.
4. **Confirm result.** Report what was done and present next-step options.
...
A thread file saved to `.temp/threads/` with the format above, or a summary/listing displayed to the user depending on the action.

## Adjacent Skills

- `/devkit:session-handoff` for handing off session state to a new session
- `/devkit:plan-track` for monitoring plan progress within a thread
