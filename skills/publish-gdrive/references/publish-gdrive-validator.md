# publish-gdrive Validator

The validator gate `adk-publish-gdrive` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/publish-gdrive-<slug>-validator.md`.
...
| --- | --- | --- |
| google-drive MCP auth | MCP authenticated OR `GOOGLE_*` creds present + scope OK | BLOCKER otherwise |
| Folder resolved | Destination folder ID resolved | BLOCKER otherwise |
| Format chosen | Google Doc / plain markdown file / PDF — explicit | BLOCKER if unclear |
...
## Phase 3: Pre-publish validation (Google Drive action readiness)

Run immediately before the Drive MCP / REST write; every check must pass.

| Check | Pass criteria | Evidence captured |
| --- | --- | --- |
| Sharing setting decided | Public / org-wide / specific viewers — chosen explicitly per request | Sharing setting |
| No secrets in body | Body scanned for tokens / passwords / API keys | Secret-scan grep |
| Idempotency | Do not duplicate an existing file with same name in same folder unless explicit | Existence check |
...
| --- | --- | --- |
| File created / updated | Returned file ID + URL | File URL |
| Read-back verified | Drive API confirms the file exists with expected name + permissions | Read-back response |
...
The validator writes its check log to `.temp/notes/publish-gdrive-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/publish-gdrive-<slug>.md
```
