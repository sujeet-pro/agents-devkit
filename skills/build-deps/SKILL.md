...
- Major-version upgrade with breaking API -> `@adk:build-migrate` (a.k.a. `adk-build-migrate`)
- Adding a new dep as part of a feature -> `@adk:build-feature` (a.k.a. `adk-build-feature`) (and call this skill if the dep choice itself is non-trivial)
- Audit of dependency health (no changes) -> `@adk:audit-repo` (a.k.a. `adk-audit-repo`)
