...
Dependency table (name / current / target / reason / risk) + recommended action per row + advisories list.
...
Add: per-dep changelog summary for upgrades, transitive-dep diff, license report, supply-chain risk per package, removal blast radius for prune.
