...
name: adk-improve
description: |
  Improve, learn, refresh-metadata, update-defaults, self-improve, train-skill, learn-from-session. The self-improvement loop. Always interactive (asks first: defaults from decision logs, metadata via MCP introspection, or both). Defaults: runs scripts/proposal_generator.py against `$ADK_MEMORY_HOME/learning/decisions.jsonl`, drafts updates to `core.json5.defaults.*`, presents each with ≥3 evidence lines, applies on confirm, then rotates decisions.jsonl to archive/ and appends to summary.md. Metadata: runs scripts/metadata_introspector.py to refresh `$ADK_DATA_HOME/metadata/<source>.json` from every reachable MCP. Bounded: cannot change shared/constitution.md or the Must-do/Must-not-do sections of any SKILL.md (constitution-grade). Per-item confirmation regardless of mode; never auto-applies. Min-evidence default 3. Manual-only by design (disable-model-invocation: true) so it doesn't auto-trigger mid-session.
