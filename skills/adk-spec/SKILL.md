...
compatibility: Self-contained published skill for npx skills. Works best when git and python3 are available. Web access recommended for research. For ambiguous spec requests, it prefers the `brainstorming` MCP server and falls back to the shared manual workflow when unavailable.
user-invocable: true
argument-hint: "<topic> [--type prd|technical|api|feature|acceptance] [--scope <path>] [--help]"
workflow-tier: full
