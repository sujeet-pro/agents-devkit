---
name: spec-analyze
description: "Use when you need to verify consistency between specifications, plans, tasks, and implementation without modifying any artifacts"
user_invocable: true
arguments:
  - name: path
    description: "Path to .temp/specs/ or .temp/plans/ directory, or specific artifact file"
    required: true
  - name: depth
    description: "Analysis depth: quick, standard, thorough (default: standard)"
    required: false
  - name: mode
...
Use `skills/_references/agentic-teams.md`, `skills/_references/review-pipeline.md`, and `skills/_references/preflight-validations.md`.

This skill is **read-only**. It detects issues across specifications, plans, tasks, and implementation code but does NOT modify any artifacts. All changes are the user's responsibility after reviewing findings.

## Preflight

Before starting analysis, run:

`zsh scripts/check-skill-deps.zsh spec-analyze`
...
Build an artifact inventory before launching child agents. If no specs or plans are found at the target path, stop and suggest `/devkit:spec-write` or `/devkit:plan-write` to create them.
...
When `mode` is `interactive` (the default), present each finding to the user one at a time:
...
When `mode` is `auto-approve`, skip the interactive loop. Accept all findings automatically and proceed directly to the traceability matrix and report.
...
- `/devkit:spec-write` for creating or revising specifications
- `/devkit:plan-write` for creating or revising execution plans
- `/devkit:checklist-generate` for requirements quality validation
- `/devkit:constitution-write` for defining project principles and constraints
