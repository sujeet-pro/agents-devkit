...
name: adk-pr-review
description: |
  Deep PR review: tree-sitter AST chunking + ollama embeddings + LanceDB hybrid (vector + BM25) retrieval + SCIP cross-file symbols + harness-LLM reranker + feature-flow tracing. Triggers on a GitHub or Bitbucket Cloud pull-request URL, OR no arg — then the next eligible row from `$ADK_CONFIG_HOME/pr-queue.json5` is atomically claimed (FIFO, 30-min auto-expiring lock so two terminals review different PRs). Curate the queue via `adk pr-scan`; a passed URL already queued merges its `slack` + `supporting_docs` into context. **Global skill** — runs from anywhere; isolates to `$ADK_DATA_HOME/skill-pr-review/<repo>_pr-<n>/`; never touches the cwd. `--detailed` controls retrieval inputs (embed model: `nomic-embed-text`, or `bge-m3`); `--deep` controls the review model profile (auto for large PRs). In `-i`/`--interactive` the user triages each finding accept/reject/edit before posting via adk-mcp-github / adk-mcp-bitbucket after explicit confirmation. Heavyweight — for lightweight review use `/adk-review`.
...
3. **Read existing PR comments (`pr-comments.json`) — and classify every thread.** For each thread (no exceptions), emit one `existing_comment_actions[]` entry. The three rules (see `references/comment-resolution.md`):
   ...
   - **Open or Resolved + an acceptable reply explains the disposition** (offline-aligned, "done in the next PR", "tracked in PROJ-1234", "synced with @alice" / "as discussed with Sujeet") → `decision: "leave-as-is"`.
