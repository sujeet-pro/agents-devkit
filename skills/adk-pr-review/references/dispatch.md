...
The slug under `$ADK_DATA_HOME/skill-pr-review/` is `<repo>_pr-<n>`. If two PRs across hosts share the same `<repo>_pr-<n>` name, the second invocation gets `<repo>_pr-<n>-2`. The host is recorded in `pr.json.host` so downstream scripts pick the right fetcher.
