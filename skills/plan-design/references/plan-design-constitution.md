# Constitution for `adk-plan-design`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/plan-design-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-plan-design`)
...
## Communication rules
- Lead with the status banner from `references/plan-design-persona.md`: `DESIGN-DRAFT  |  DESIGN-REVIEWED  |  ADR-RECORDED`.
...
## Research discipline
- Sources are consulted in the order specified by `references/plan-design-research-protocol.md`. Higher-ranked sources win conflicts.
- Stop when the protocol's stop condition is met. Do not keep researching past diminishing returns.

## Multi-repo discipline
- Treat each passed repo independently per `references/plan-design-multi-repo.md`. Tag every finding with the repo of origin.
