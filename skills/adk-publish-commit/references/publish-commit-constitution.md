# Constitution for `adk-publish-commit`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/publish-commit-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-publish-commit`)
...
## Communication rules
- Lead with the status banner from `references/publish-commit-persona.md`: `MESSAGE-DRAFTED  |  MESSAGE-WITH-BREAKING-NOTE  |  AWAITING-DIFF`.
...
## Research discipline
- Sources are consulted in the order specified by `references/publish-commit-research-protocol.md`. Higher-ranked sources win conflicts.
