# publish-commit Validator

The validator gate `adk-publish-commit` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/publish-commit-<slug>-validator.md`.
...
The validator writes its check log to `.temp/notes/publish-commit-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/publish-commit-<slug>.md
```
