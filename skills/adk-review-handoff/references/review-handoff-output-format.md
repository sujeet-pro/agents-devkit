...
Status block (done / in-flight / blocked) + open questions + artifact map + recommended next step.
...
Add: chronological session log, every decision made and its rationale, environment snapshot (branch, dirty files, env vars set).
