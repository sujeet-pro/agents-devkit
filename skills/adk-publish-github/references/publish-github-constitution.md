# Constitution for `adk-publish-github`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/publish-github-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-publish-github`)
...
## Communication rules
- Lead with the status banner from `references/publish-github-persona.md`: `ACTION-DONE <url>  |  ACTION-FAILED <reason>  |  AWAITING-APPROVAL (merge/destructive)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/publish-github-research-protocol.md`. Higher-ranked sources win conflicts.
