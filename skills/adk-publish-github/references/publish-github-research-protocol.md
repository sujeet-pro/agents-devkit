...
`adk-publish-github` consults sources in the order below. Higher-ranked sources win conflicts. Stop researching when the stop condition is met — diminishing returns past that point.

## Sources, in order

1. gh auth status (must be OK) or MCP server availability.
2. git remote get-url origin (to confirm github.com host).
3. Existing PR/issue state (gh pr view / issue view) before write.

## Stop condition
Action verified to have landed via API read-back.
