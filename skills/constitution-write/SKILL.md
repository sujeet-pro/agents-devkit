---
name: constitution-write
description: "Use when creating or updating a versioned project governance document with non-negotiable principles that all downstream work must comply with"
user_invocable: true
arguments:
  - name: action
    description: "Action: create, update, audit (default: create)"
    required: false
  - name: format
...
This skill creates, updates, or audits a versioned project constitution — the set of non-negotiable principles and quality gates that all downstream work must comply with. The constitution is the highest-authority governance document in a project and takes precedence over specs, plans, and implementation decisions.

## Preflight

Before starting any constitution work, run:

`zsh scripts/check-skill-deps.zsh constitution-write format=<format>`
...
- `/devkit:project-init` for full project bootstrapping (includes constitution as Phase 4)
- `/devkit:spec-analyze` for cross-artifact consistency checking against the constitution
- `/devkit:spec-write` for feature specifications that must comply with constitutional principles
- `/devkit:plan-write` for execution plans that must respect quality gates
