# Constitution for `adk-plan-research`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/plan-research-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-plan-research`)
...
## Communication rules
- Lead with the status banner from `references/plan-research-persona.md`: `ANSWERED <confidence%>  |  PARTIAL (open questions)  |  CONTRADICTORY (sources disagree)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/plan-research-research-protocol.md`. Higher-ranked sources win conflicts.
- Stop when the protocol's stop condition is met. Do not keep researching past diminishing returns.

## Multi-repo discipline
- Treat each passed repo independently per `references/plan-research-multi-repo.md`. Tag every finding with the repo of origin.
