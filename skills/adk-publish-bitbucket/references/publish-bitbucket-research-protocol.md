...
`adk-publish-bitbucket` consults sources in the order below. Higher-ranked sources win conflicts. Stop researching when the stop condition is met — diminishing returns past that point.

## Sources, in order

1. Bitbucket auth (env vars / app-password) or MCP server availability.
2. git remote (must be bitbucket.org).
3. Existing PR/issue state before write.
