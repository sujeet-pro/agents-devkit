# publish-bitbucket Validator

The validator gate `adk-publish-bitbucket` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/publish-bitbucket-<slug>-validator.md`.
...
| --- | --- | --- |
| Action chosen | create-pr / update-pr / comment / task-create / task-resolve / merge — explicit | BLOCKER if unclear |
| Bitbucket auth | Bitbucket MCP authenticated OR `BITBUCKET_USER` + `BITBUCKET_APP_PASSWORD` present + scope OK | BLOCKER otherwise — point at install instructions |
| Workspace + repo resolved | `<workspace>/<repo>` resolved from URL or origin | BLOCKER otherwise |
| Never-auto guard | If action is `pr-merge` / force-push / task-resolve without code change: explicit user approval | BLOCKER without approval |
...
## Phase 3: Pre-publish validation (Bitbucket action readiness)

Run immediately before the Bitbucket MCP / REST write; every check must pass.

| Check | Pass criteria | Evidence captured |
| --- | --- | --- |
| Payload validated | Title / body / reviewers / source / destination all valid | Per-field check |
| No secrets in body | Body scanned for tokens / passwords / API keys | Secret-scan grep |
| Task / comment idempotency | Do not create duplicate task or comment for same content + location | Existence check |
...
| --- | --- | --- |
| Action succeeded | MCP / REST returned the expected id / URL | Returned id / URL |
| Read-back verified | Bitbucket API confirms the change landed | Read-back diff |
...
The validator writes its check log to `.temp/notes/publish-bitbucket-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/publish-bitbucket-<slug>.md
```
