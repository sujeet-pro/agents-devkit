# Constitution for `adk-publish-bitbucket`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/publish-bitbucket-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-publish-bitbucket`)
...
## Communication rules
- Lead with the status banner from `references/publish-bitbucket-persona.md`: `ACTION-DONE <url>  |  ACTION-FAILED <reason>  |  AWAITING-APPROVAL (merge)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/publish-bitbucket-research-protocol.md`. Higher-ranked sources win conflicts.
