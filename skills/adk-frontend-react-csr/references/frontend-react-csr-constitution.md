# Constitution for `adk-frontend-react-csr`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/frontend-react-csr-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-frontend-react-csr`)
...
## Communication rules
- Lead with the status banner from `references/frontend-react-csr-persona.md`: `BOOTSTRAPPED  |  FEATURE-LANDED  |  AUDIT-DONE  |  STACK-DRIFT (flagged)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/frontend-react-csr-research-protocol.md`. Higher-ranked sources win conflicts.
