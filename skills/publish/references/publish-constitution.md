...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/publish-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-publish`)
...
## Communication rules
- Lead with the status banner from `references/publish-persona.md`: `ROUTED <publish-task>`.
