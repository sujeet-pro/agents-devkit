# visualize-chart Validator

The validator gate `adk-visualize-chart` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/visualize-chart-<slug>-validator.md`.
...
| --- | --- | --- |
| Chart type fits the data | Bar / line / pie / scatter / histogram / stacked area / heatmap — picked per question | BLOCKER if mismatched (e.g., pie for trend over time) |
| Data source readable | CSV / JSON / SQL / REST source resolves and parses | BLOCKER otherwise |
| Editable spec path chosen | Where the editable plot spec lives is decided | BLOCKER without |
...
## Phase 3: Pre-handoff validation (chart quality)

Run after the chart is rendered; verify it renders, the data is right, and the chart type fits the question.

| Check | Pass criteria | Evidence captured |
| --- | --- | --- |
| Spec file written | Editable plot spec (vega-lite JSON / matplotlib script / etc.) saved alongside output | File presence |
| Renders without error | Engine rendered the spec to the target format (svg / png) cleanly | Render command output |
| Axes labeled | X / Y axes labeled with units; legend present where applicable | Per-axis check |
| No misleading visuals | Truncated y-axes flagged; pie slices ≤ 5; color-blind-safe palette where applicable | Per-rule check |
...
| --- | --- | --- |
| Output + spec returned | Both spec AND rendered output paths | File paths |
...
The validator writes its check log to `.temp/notes/visualize-chart-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/visualize-chart-<slug>.md
```
