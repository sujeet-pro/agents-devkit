# Constitution for `adk-visualize-chart`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/visualize-chart-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-visualize-chart`)
...
## Communication rules
- Lead with the status banner from `references/visualize-chart-persona.md`: `CHART-RENDERED <path>  |  CHART-DATA-MISSING`.
...
## Research discipline
- Sources are consulted in the order specified by `references/visualize-chart-research-protocol.md`. Higher-ranked sources win conflicts.
