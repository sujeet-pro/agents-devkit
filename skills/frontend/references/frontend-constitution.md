...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/frontend-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-frontend`)
...
## Communication rules
- Lead with the status banner from `references/frontend-persona.md`: `ROUTED <frontend-task>`.
