...
name: build-api
description: |
  Design or evolve a stable interface — REST/HTTP API, RPC contract, internal module boundary, library export surface, CLI flag set — using contract-first discipline, Hyrum's Law awareness, the One-Version Rule, and edge-only validation. Use when the deliverable is the SHAPE of an interface that others (services, teams, customers, future code) will couple to. Different from `@adk:build-feature` (a.k.a. `adk-build-feature`) which implements behavior behind an existing interface, and from `@adk:plan-design` (a.k.a. `adk-plan-design`) which is the architecture write-up that may include an API as one component. Do not use for implementation-only changes that don't touch the contract (use `@adk:build-feature`), framework migrations (use `@adk:build-migrate`), or removing an existing API (use `@adk:build-migrate` with the deprecation reference).
metadata:
  category: build
  kind: task
  layer: 4
