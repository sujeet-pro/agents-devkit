...
`adk-build-refactor` consults sources in the order below. Higher-ranked sources win conflicts. Stop researching when the stop condition is met — diminishing returns past that point.

## Sources, in order

1. Repo: target code, all callers, all tests covering the targets.
2. Lint/format config so the refactor matches house style.
3. Recent git log to avoid colliding with in-flight feature work.

## Stop condition
Test output is provably identical and lint/type-check pass.
