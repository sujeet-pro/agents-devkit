...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/build-refactor-artifact-format.md` for the path matrix.
...
## Communication rules
- Lead with the status banner from `references/build-refactor-persona.md`: `REFACTOR-DONE (tests identical)  |  REFACTOR-PARTIAL  |  REVERTED (behavior changed)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/build-refactor-research-protocol.md`. Higher-ranked sources win conflicts.
