# Constitution for `adk-plan-roadmap`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/plan-roadmap-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-plan-roadmap`)
...
## Communication rules
- Lead with the status banner from `references/plan-roadmap-persona.md`: `ROADMAP-DRAFT  |  ROADMAP-APPROVED  |  BLOCKED-ON <step>`.
...
## Research discipline
- Sources are consulted in the order specified by `references/plan-roadmap-research-protocol.md`. Higher-ranked sources win conflicts.
