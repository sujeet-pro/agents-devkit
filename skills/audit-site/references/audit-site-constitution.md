# Constitution for `adk-audit-site`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/audit-site-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-audit-site`)
...
## Communication rules
- Lead with the status banner from `references/audit-site-persona.md`: `AUDIT-DRAFT  |  AUDIT-FINAL <perf>/<a11y>/<seo>/<best-practices> scores + findings`.
...
## Research discipline
- Sources are consulted in the order specified by `references/audit-site-research-protocol.md`. Higher-ranked sources win conflicts.
