...
def test_update_all_with_empty_queue(tmp_path, capsys):
    ...
    # Message wording was widened to cover declined rows too.
...
def test_update_all_skips_merged_and_continues_on_failure(tmp_path, capsys, monkeypatch):
    ...
    queue_path = _write_queue(tmp_path, [
        {"pr_link": "https://github.com/acme/foo/pull/1", "status": STATUS_PENDING},
        {"pr_link": "https://github.com/acme/foo/pull/2", "status": STATUS_MERGED},
        {"pr_link": "https://github.com/acme/foo/pull/3", "status": STATUS_PENDING},
        {"pr_link": "https://github.com/acme/foo/pull/4", "status": STATUS_PENDING},
    ])
    calls: list[str] = []
    ...
    def fake_refresh(pr_url, entry, *, queue_path, log):
        ...
        if pr_url.endswith("/3"):
            return {"pr_url": pr_url, "status": "failed", "stage": "meta",
                    "reason": "simulated"}
        return {"pr_url": pr_url, "refreshed": "meta", "head_oid": "abc"}
...
def test_refresh_one_metadata_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pr_scan, "cheap_pr_meta",
                        lambda url, log: {"head_oid": "newhead", "merged_at": None,
                                          "state": "OPEN"})
    queue_path = _write_queue(tmp_path, [
        {"pr_link": "https://github.com/acme/foo/pull/1", "status": STATUS_PENDING,
         "head_oid": "oldhead"},
    ...
    log = get_logger("test")
    entry = {"pr_link": "https://github.com/acme/foo/pull/1", "head_oid": "oldhead"}
