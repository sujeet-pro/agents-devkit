...
Generate diagrams using Mermaid syntax. Supports all 20+ Mermaid v11 diagram types. This stage writes a `.mermaid` source file only -- use `diagramkit render` to produce images.
...
If `--type` is not specified, auto-detect from the description (see the parent SKILL.md for detection rules). If `--type` is specified, use it directly.
