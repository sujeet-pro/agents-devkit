...
This stage generates "unit tests for English" -- quality checks that validate REQUIREMENTS, not implementation. Every checklist item is a concrete question about the specification itself: is it complete, clear, consistent, and testable? The output is a traceable checklist that catches gaps, ambiguities, and contradictions before a single line of code is written.
...
When `--interactive` is `interactive` (default), present each checklist item one at a time:
...
When `--interactive` is `auto-approve`, skip the interactive loop. Instead, auto-assess each checklist item by re-reading the relevant spec section:
...
- If quality score < 50: "Specification requires significant rework. Recommend running write mode to rebuild from the ground up."

If any failed items exist, ask:

```text
Action: [R]evise spec (generate amendment notes) | [E]xport checklist only | [H]andoff to write mode
```
...
- Handoff: recommend the user run write mode with the checklist as input context.
