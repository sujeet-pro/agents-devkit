# Constitution for `adk-docs-write`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/docs-write-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-docs-write`)
...
## Communication rules
- Lead with the status banner from `references/docs-write-persona.md`: `DOC-DRAFT  |  DOC-FINAL  |  GAPS-FOUND-NEED-FIX`.
...
## Research discipline
- Sources are consulted in the order specified by `references/docs-write-research-protocol.md`. Higher-ranked sources win conflicts.
- Stop when the protocol's stop condition is met. Do not keep researching past diminishing returns.

## Multi-repo discipline
- Treat each passed repo independently per `references/docs-write-multi-repo.md`. Tag every finding with the repo of origin.
