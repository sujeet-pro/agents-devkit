# Constitution for `adk-review-local`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/review-local-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-review-local`)
...
## Communication rules
- Lead with the status banner from `references/review-local-persona.md`: `REVIEW-DRAFT  |  READY-TO-COMMIT (no Blocker/Critical)  |  BLOCKED on <finding>`.
...
## Research discipline
- Sources are consulted in the order specified by `references/review-local-research-protocol.md`. Higher-ranked sources win conflicts.
