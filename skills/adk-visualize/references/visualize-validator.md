...
The validator gate `adk-visualize` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/visualize-<slug>-validator.md`.

For a category router, the validator is lightweight: confirm the routing decision is justified by the user's task shape and that the picked task skill exists and is reachable.
...
| Report written | `.temp/reports/visualize-<slug>.md` (or this skill's analogue) exists with full content | File path + size |
| Validator log written | `.temp/notes/visualize-<slug>-validator.md` exists with all four phases' outcomes | File path + size |
...
The validator writes its check log to `.temp/notes/visualize-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/visualize-<slug>.md
```
