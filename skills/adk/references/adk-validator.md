...
The validator gate `adk-adk` MUST run at every phase boundary and before declaring the run complete. Skipping the validator is a hard rule violation.

The validator has four phases. Each phase has explicit checks with `BLOCKER` / `WARN` / `OK` outcomes. The validator's check log lives at `.temp/notes/adk-<slug>-validator.md`.
...
| Report written | `.temp/reports/adk-<slug>.md` (or this skill's analogue) exists with full content | File path + size |
| Validator log written | `.temp/notes/adk-<slug>-validator.md` exists with all four phases' outcomes | File path + size |
...
The validator writes its check log to `.temp/notes/adk-<slug>-validator.md` for audit. Format:
...
Final report: .temp/reports/adk-<slug>.md
```
