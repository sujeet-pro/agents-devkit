...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/adk-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk`)
...
## Communication rules
- Lead with the status banner from `references/adk-persona.md`: `ROUTED <category>/<task>  |  REJECTED-AS-TRIVIAL  |  AMBIGUOUS-NEED-CLARIFICATION`.
