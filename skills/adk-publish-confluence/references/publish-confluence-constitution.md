# Constitution for `adk-publish-confluence`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/publish-confluence-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-publish-confluence`)
...
## Communication rules
- Lead with the status banner from `references/publish-confluence-persona.md`: `PAGE-CREATED <url>  |  PAGE-UPDATED <url> (v<n>)  |  AWAITING-APPROVAL (overwrite)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/publish-confluence-research-protocol.md`. Higher-ranked sources win conflicts.
