...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/docs-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-docs`)
...
## Communication rules
- Lead with the status banner from `references/docs-persona.md`: `ROUTED <docs-task>`.
