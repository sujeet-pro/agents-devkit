...
## Phase 3: Pre-report / pre-post validation

Run after the dimension passes complete but before issuing the report (or posting comments, if applicable).
...
| Report written | `.temp/reports/review-feedback-<slug>.md` (or this skill's analogue) exists with full content | File path + size |
| Validator log written | `.temp/notes/review-feedback-<slug>-validator.md` exists with all four phases' outcomes | File path + size |
