...
compatibility: Self-contained published skill for npx skills. Works best when git, node, npx, and python3 are available and diagramkit is installed locally or globally.
user-invocable: true
argument-hint: "<diagram-request> [--doc <path/to/doc.md>] [--name <slug>] [--engine mermaid|excalidraw|drawio|graphviz] [--type <diagram-type>] [--help]"
workflow-tier: full
...
- The inserted or replaced markdown embed is correctly placed and readable
- If rendering could not run, report explicitly and leave source in a recoverable state
...
```
/adk-diagram add a system architecture diagram --doc README.md --engine mermaid
```

### Update an existing flow diagram
```
/adk-diagram update the auth flow to include the new OAuth provider --doc docs/auth.md --name auth-flow
```

### Create an ER diagram with Graphviz
```
/adk-diagram create an ER diagram for the billing schema --doc docs/billing.md --engine graphviz --type er --auto
```
