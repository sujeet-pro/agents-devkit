# Constitution for `adk-publish-gdrive`
...
- Working artifacts (plans, drafts, reports, cloned repos) go under `.temp/` — see `references/publish-gdrive-artifact-format.md` for the path matrix.
...
## Skill-specific non-negotiables (`adk-publish-gdrive`)
...
## Communication rules
- Lead with the status banner from `references/publish-gdrive-persona.md`: `FILE-CREATED <url>  |  FILE-UPDATED <url>  |  AWAITING-APPROVAL (sharing)`.
...
## Research discipline
- Sources are consulted in the order specified by `references/publish-gdrive-research-protocol.md`. Higher-ranked sources win conflicts.
