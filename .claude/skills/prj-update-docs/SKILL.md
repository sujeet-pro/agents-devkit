...
This is a project-local skill loaded only when working in this repo (it lives at
`.claude/skills/prj-update-docs/SKILL.md`). Invoke it as a regular Claude Code skill — it is
not namespaced under the `adk` plugin because it is project-scoped, not plugin-scoped:

```text
/prj-update-docs                # full interactive run
/prj-update-docs --auto         # unattended; pick safe defaults
/prj-update-docs --mode review  # report drift only, no writes
/prj-update-docs --mode fix     # apply doc + diagram fixes
/prj-update-docs --scope skills # limit to one artifact category
```
