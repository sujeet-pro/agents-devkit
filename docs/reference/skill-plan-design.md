...
- Direction is still unsettled -> `@adk:plan-brainstorm` (a.k.a. `adk-plan-brainstorm`)
- The deliverable is a product spec, not an architecture -> `@adk:plan-spec` (a.k.a. `adk-plan-spec`)
- Feature-scale work needing only an ordered plan -> `@adk:plan-roadmap` (a.k.a. `adk-plan-roadmap`)
- A UI / interaction design -> `@adk:frontend-design` (a.k.a. `adk-frontend-design`)
...
3. **Draft** - write the design section by section using the template below. Include at least one diagram in mermaid (delegate to `@adk:visualize-diagram` (a.k.a. `adk-visualize-diagram`) if the diagram is complex).
...
- Putting implementation code in the design doc. Save for `@adk:build-feature` (a.k.a. `adk-build-feature`).
