...
- Direction is still ambiguous -> `@adk:plan-brainstorm` (a.k.a. `adk-plan-brainstorm`) first.
- The work is feature-scale; only a short ordered plan is needed -> `@adk:plan-roadmap` (a.k.a. `adk-plan-roadmap`).
- The deliverable is an architecture / system design -> `@adk:plan-design` (a.k.a. `adk-plan-design`).
- The spec is for an existing public API doc -> `@adk:docs-write` (a.k.a. `adk-docs-write`).
...
- Drafting code in the spec. Save it for `@adk:build-feature` (a.k.a. `adk-build-feature`).
