...
- [`datadog`](adk-investigate-datadog.md) - Datadog hosted MCP (Preview). Auth via DATADOG_API_KEY + DATADOG_APP_KEY (App key needs scope `mcp_read`; add `mcp_write` only if you actually need to mute/silence monitors). Legacy DD_API_KEY / DD_APP_KEY are also...
