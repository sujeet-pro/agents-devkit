...
description: 'PR, local, or branch code review — review, fix, describe, finalize with conditional stages'
...
Use `code-review-pr` to pR, local, or branch code review — review, fix, describe, finalize with conditional stages. In normal use, explicit selector flags win over inference, but the skill can still auto-detect the right path when the prompt is short.

## Overview

`code-review-pr` belongs to the `task` layer and is declared at the `full` tier with the `complex-build` workflow family. That metadata is more than labeling: it tells you how much planning happens before execution, how much the skill is allowed to infer, and whether the result should be a final artifact, a routing decision, or a shared contract for another skill.
...
```text
/adk:code-review-pr <target> --verbosity detailed
```
