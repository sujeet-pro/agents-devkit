...
description: 'Create Excalidraw diagrams — hand-drawn style architecture overviews and freeform diagrams. Full JSON format reference with light/dark mode'
...
Use `diagram-excalidraw` to create Excalidraw diagrams — hand-drawn style architecture overviews and freeform diagrams. Full JSON format reference with light/dark mode. In normal use, explicit selector flags win over inference, but the skill can still auto-detect the right path when the prompt is short.

## Overview

`diagram-excalidraw` belongs to the `task` layer and is declared at the `full` tier with the `quick-action` workflow family. That metadata is more than labeling: it tells you how much planning happens before execution, how much the skill is allowed to infer, and whether the result should be a final artifact, a routing decision, or a shared contract for another skill.
...
- Light mode: white background, standard palette colors
- Dark mode: dark surface background, adjusted colors for WCAG contrast
...
```text
/adk:diagram-excalidraw --render --format png <prompt-text>
```
