...
- Implementation after the direction is settled -> `@adk:build` (a.k.a. `adk-build`)
- Reviewing existing code or PRs -> `@adk:review` (a.k.a. `adk-review`)
- Writing user-facing or maintenance docs -> `@adk:docs` (a.k.a. `adk-docs`)
- Auditing an existing repo or site -> `@adk:audit` (a.k.a. `adk-audit`)
...
| --- | --- | --- |
| `@adk:plan-brainstorm` (a.k.a. `adk-plan-brainstorm`) | Ambiguity to close, blast radius to choose, 2-3 options to weigh, direction not yet locked | The decision is already made - skip to spec or roadmap |
| `@adk:plan-research` (a.k.a. `adk-plan-research`) | A factual question must be answered (framework behavior, upstream API, library comparison) | The answer is in this repo or already obvious - skip research |
| `@adk:plan-spec` (a.k.a. `adk-plan-spec`) | Need a written spec (PRD, RFC, functional or technical spec) before implementation | A short plan is enough - use roadmap |
| `@adk:plan-design` (a.k.a. `adk-plan-design`) | Need an architecture / system design / ADR / data model write-up | The work is feature-scale and needs only an implementation plan |
| `@adk:plan-roadmap` (a.k.a. `adk-plan-roadmap`) | Direction and design are settled; need an ordered, scoped implementation plan with milestones | Direction is still unresolved - brainstorm first |
