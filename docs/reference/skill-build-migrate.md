...
- Patch / minor upgrade with no API change -> `@adk:build-deps` (a.k.a. `adk-build-deps`)
- Single feature add or bug fix -> `@adk:build-feature` (a.k.a. `adk-build-feature`)
- Restructure with no behavior change -> `@adk:build-refactor` (a.k.a. `adk-build-refactor`)
- Test-only adjustments to keep up with a migration -> `@adk:build-test` (a.k.a. `adk-build-test`) (called from this skill)
...
2. **Research breaking changes** - pull the official upgrade / migration guide. Capture the breaking-change inventory with citations (delegate to `@adk:plan-research` (a.k.a. `adk-plan-research`) if the surface is large).
