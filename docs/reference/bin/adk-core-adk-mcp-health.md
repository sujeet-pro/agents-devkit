...
# Aliases: each canonical var (left) is satisfied if any alias (right) is set in env.
# Used to support legacy DD_* alongside canonical DATADOG_*.
ENV_VAR_ALIASES_DATADOG_API_KEY="DD_API_KEY"
ENV_VAR_ALIASES_DATADOG_APP_KEY="DD_APP_KEY"
...
env_status() {
  local var="$1"
  if [[ -n "${!var-}" ]]; then echo "present"; return; fi
  local alias_var="ENV_VAR_ALIASES_${var}"
  local alias_name="${!alias_var-}"
  if [[ -n "$alias_name" && -n "${!alias_name-}" ]]; then echo "present-via-alias"; return; fi
  echo "MISSING"
}
...
if (( JSON )); then
  ...
  if [[ "$MODE" == "full" || "$MODE" == "env" ]]; then
    ...
    for i in "${!ENV_VARS[@]}"; do
      v="${ENV_VARS[$i]}"
      s="$(env_status "$v")"
      [[ "$s" == "present-via-alias" ]] && s="needs-alias"
      printf '    "%s": "%s"' "$v" "$s"
  ...
  for v in "${ENV_VARS[@]}"; do
    ...
    case "$s" in
      present)
        printf '  - %-26s present\n' "$v" ;;
      present-via-alias)
        alias_var="ENV_VAR_ALIASES_${v}"
        a="${!alias_var}"
        printf '  - %-26s legacy %s set, but %s is unset — add to ~/.zshenv: export %s="$%s"\n' "$v" "$a" "$v" "$v" "$a" ;;
      *)
        case "$v" in
          GITHUB_PAT)                   hint='(mint at https://github.com/settings/personal-access-tokens/new)' ;;
          DATADOG_API_KEY|DATADOG_APP_KEY) hint='(see https://app.datadoghq.com/organization-settings/; legacy DD_API_KEY / DD_APP_KEY also accepted)' ;;
