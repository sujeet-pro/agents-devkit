...
- The design is settled; just implement -> `@adk:frontend-feature` (a.k.a. `adk-frontend-feature`)
- Architecture / system design write-up -> `@adk:plan-design` (a.k.a. `adk-plan-design`)
- Backend or non-UI design -> `@adk:build` (a.k.a. `adk-build`) family
- Single diagram only -> `@adk:visualize-diagram` (a.k.a. `adk-visualize-diagram`)
...
4. **5-sample mockup loop (MANDATORY in plan mode unless `--skip-design`).** Hand off to `@adk:frontend-mockup` (a.k.a. `adk-frontend-mockup`) with the loaded design system + requirements. It generates 5 distinct aesthetic-direction HTML mockups at `.temp/task-<slug>/preview/sample-{1..5}.html`. Show user. User picks ONE (or asks for 5 more variants). Locked sample = the design baseline for everything below.
5. **Decide direction** - the picked sample IS the direction. Capture rationale (why this over the other four).
6. **Layout** - sketch responsive layout for each viewport (mobile, tablet, desktop) using ASCII / mermaid / linked image, anchored on the picked sample.
7. **States** - enumerate every state for every interactive element: default, hover, focus-visible, active, disabled, loading, empty, error.
8. **Interaction spec** - keyboard map, focus order, error recovery, loading/skeleton strategy, motion (with `prefers-reduced-motion` fallback).
9. **Accessibility check** - WCAG 2.2 AA: contrast, target size (44 CSS px), focus visibility, ARIA only when needed, content meaning preserved without color.
10. **Browser-validate the picked sample.** Hand off to `@adk:validate-browser` (a.k.a. `adk-validate-browser`) with `--mode visual-check --target file://.../sample-N.html` to confirm it renders correctly at 360/768/1280 and `--mode a11y-audit` to confirm WCAG 2.2 AA.
11. **Pre-delivery checklist** - walk every item in `frontend-design-pre-delivery-checklist.md`; surface unfinished items in the report.
12. **Validate (per `frontend-design-validator.md`)** - run the four-phase validator gate; capture evidence in `.temp/notes/frontend-design-<slug>-validator.md` before the final report.
13. **Self-review** - run the validation list below.
14. **Report** - return the file path, 3-bullet TL;DR, viewports covered, picked sample number + rationale, follow-up questions for the user.
