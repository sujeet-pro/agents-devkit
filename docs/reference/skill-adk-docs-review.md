...
Standalone task skill under the `adk-docs` category router. Produces a findings-first review of an existing document with each finding anchored to the doc and to the source-of-truth it claims to describe. In `--mode confluence`, also reconciles existing comments and posts inline + footer comments back to the live page.
...
- Publishing a Markdown doc to Confluence → `adk-publish-confluence`
- Reviewing code, not docs → `adk-review-pr` / `adk-review-local`
- Auditing a whole repo (which can call this skill per-doc) → `adk-audit-repo`
