---
title: 'adk-review-local-changes'
...
skill_name: adk-review-local-changes
category: task
workflow_tier: full
user_invocable: true
---
...
Use `adk-review-local-changes` to review local uncommitted or local branch changes before commit or PR. Use when the work exists locally and needs a pre-submit review. In normal use, explicit selector flags win over inference, but the skill can still auto-detect the right path when the prompt is short.

## Overview

`adk-review-local-changes` belongs to the `task` layer and is declared at the `full` tier with the `standard-task` workflow family. That metadata is more than labeling: it tells you how much planning happens before execution, how much the skill is allowed to infer, and whether the result should be a final artifact, a routing decision, or a shared contract for another skill.
