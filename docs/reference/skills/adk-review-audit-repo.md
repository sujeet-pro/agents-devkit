---
title: 'adk-review:audit-repo'
description: '  Whole-repo multi-dimensional audit — security, performance, code quality, dependencies, test coverage, architecture — producing a single severity-tiered report with file-anchored evidence per finding. Read-only: NEVER opens PRs, NEVER pushes, NEVER mutates code. Inventory first (language / framework / dep manager / test framework / lint tool / CI provider), then per-dimension passes (run repo-native tools first; sample architecture by reading the largest files), then aggregate top-10 + per-dimension detail. Includes explicit "what''s healthy" findings alongside problems. Use when the deliverable is a multi-file audit report (e.g. "what''s the security posture of acme/checkout-api?", "audit the storefront repo before the M&A", "are we ready to open-source this?"). Do NOT use for PR review (use `review-pr`), single-PR sanity (use `audit-pr`), or any kind of fix work (use `code-*` skills).'
plugin: 'adk-review'
skill: 'audit-repo'
source: 'plugins/adk-review/skills/audit-repo/SKILL.md'
group: 'review-skills'
order: 2102
---
# adk-review:audit-repo

## Source

`plugins/adk-review/skills/audit-repo/SKILL.md`
