# P8 · TUI — α + β + γ + δ + θ + ζ + η slices (2026-05-21 → 2026-05-22)

Status: **SHIPPED locally** (read-only viewer + run-sync + single-PR review + worker visibility + multi-select batch + add-PR + repo management). Sub-phases ε / ι / κ / λ pending.
...
- **Repo-wide: 517 tests, 517 passing.** Pre-existing `test_quiet_hours_aborts_inside_window` red was fixed at the start of Session 4 (commit `a5b9968`).
...
1. **ε — Phase-aware progress bars + ETA from heartbeat polling.** Reads the worker's `<pid>.json` `current_phase` field to render a progress indicator per row in the QueueTable. Builds directly on δ + θ (writer + reader already in place). Lighter than ι.
2. **κ — Agent picker + headless fallback.** Lets the user choose `claude` / `codex` / other via a registry; the worker already takes `--agent-bin`. Headless fallback: `adk worker --headless` for CI use.
3. **ι — End-of-run recap modal.** After batch finishes, a modal summarising N reviewed / N posted / N skipped / errors.
4. **λ — Polish (themes, reattach-on-restart, detach prompt).** The final slice of P8.
