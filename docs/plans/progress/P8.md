# P8 · TUI — α + β + γ slices (2026-05-21 → 2026-05-22)

Status: **SHIPPED locally** (read-only viewer + run-sync from TUI). Sub-phases δ–λ pending.
...
- Repo-wide: **457 tests, 457 passing**. Pre-existing `test_quiet_hours_aborts_inside_window` red was fixed at the start of Session 4 (commit `a5b9968` — pinned `_in_quiet_hours` mock so the test isn't wall-clock dependent).
...
- `r` review, `n` next — shown as `(disabled)` in the footer until sub-phase δ lands.
...
1. **δ — `r` for highlighted PR runs a worker.** Needs `tui/worker.py` (the worker driver from v4 plan §8.5) — claim → spawn agent → heartbeat → release. Single-PR first; parallelism is ζ. This is the natural next slice — γ has the subprocess + log-pane substrate in place; δ adds the heartbeat-aware worker driver on top.
2. **η — Add-PR modal + repo management screen.** Self-contained UX; no new substrate required.
3. **ε — Phase-aware progress bars + ETA from `state.json` polling.** Lighter than δ; could land in parallel.
