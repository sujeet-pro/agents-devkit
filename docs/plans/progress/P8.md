# P8 · TUI — α + β + γ + δ + θ + ζ slices (2026-05-21 → 2026-05-22)

Status: **SHIPPED locally** (read-only viewer + run-sync + single-PR review + worker visibility + multi-select batch). Sub-phases ε / η / ι / κ / λ pending.
...
- **Repo-wide: 497 tests, 497 passing.** Pre-existing `test_quiet_hours_aborts_inside_window` red was fixed at the start of Session 4 (commit `a5b9968`).
...
1. **η — Add-PR modal + repo management screen.** Self-contained UX; no new substrate required. Adds `+` binding for "add PR" (URL or shorthand) and `b` to switch to repos view.
2. **ε — Phase-aware progress bars + ETA from `state.json` polling.** Lighter than η; could land in parallel. Reads the worker's heartbeat file for `current_phase` to render a progress indicator per row.
3. **κ — Agent picker + headless fallback.** Lets the user choose `claude` / `codex` / other via a registry; the worker already takes `--agent-bin`. Headless fallback: `adk worker --headless` for CI use.
4. **ι — End-of-run recap modal.** After batch finishes, a modal summarising N reviewed / N posted / N skipped / errors.

Remaining ζ-tier follow-ups (none load-bearing):
- `test_R_respects_parallel_cap` sampling rate is 100 ms; brief 2-worker windows could escape detection. Lower to 20 ms or instrument `_start_next` directly if it ever flakes on CI.
- Per-worker `_reload(force=True)` at the end of each `_run_review` (∼9 reloads back-to-back when 8 workers finish a batch). Single end-of-batch reload would suffice; <20 ms wasted today.
