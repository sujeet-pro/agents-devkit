...
Status: **SHIPPED locally** (read-only viewer + run-sync + single-PR review + worker visibility). Sub-phases ε / ζ / η / ι / κ / λ pending.
...
- **Repo-wide: 487 tests, 487 passing.**
...
1. **ζ — Multi-select + ordered batch run** (`R` / `space` to toggle; "Parallel: N" picker). Builds directly on δ — the worker driver is reusable; the TUI maintains a small pool of `_review_procs`. With θ's WorkersPane in place, the user already has the right substrate to see multiple workers at once. Highest user-visible jump.
2. **η — Add-PR modal + repo management screen.** Self-contained UX; no new substrate required.
3. **ε — Phase-aware progress bars + ETA from `state.json` polling.** Lighter than ζ; could land in parallel.
4. **κ — Agent picker + headless fallback.** Lets the user choose `claude` / `codex` / other via a registry; the worker already takes `--agent-bin`.
