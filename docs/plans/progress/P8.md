# P8 · TUI — α + β + γ + δ + θ + ζ + η + ε + κ slices (2026-05-21 → 2026-05-22)

Status: **SHIPPED locally** (read-only viewer + run-sync + single-PR review + worker visibility + multi-select batch + add-PR + repo management + phase-aware progress + agent picker). Sub-phases ι / λ pending.
...
- **Repo-wide: 552 tests, 552 passing.** 4/5 consecutive TUI-suite runs deterministically green; 1/5 surfaced the pre-existing `test_R_skips_unready_rows` flake (`NoMatches: No nodes match 'QueueTable' on Screen(id='_default')`) — flagged by the ε reviewer and tracked as a follow-up. Pre-existing `test_quiet_hours_aborts_inside_window` red was fixed at the start of Session 4 (commit `a5b9968`).
...
1. **ι — End-of-run recap modal.** After a batch finishes, push a modal summarising N reviewed / N posted / N skipped / errors with last-line context per row. Currently the batch just logs `(batch done — N rows)` without a focused summary.
2. **λ — Polish (themes, reattach-on-restart, detach prompt).** The final slice of P8.
3. **Stabilise `test_batch_run.py` flake.** With κ landed, the suite is 552 tests; the pre-existing `test_R_skips_unready_rows` (and occasionally `test_R_respects_parallel_cap`) intermittently fails with `NoMatches: No nodes match 'QueueTable' on Screen(id='_default')`. Reproduces ~1-in-5 full-suite runs but isolated test runs pass. Suspect: race between Textual mount and the test's `app.query_one(QueueTable)` query right after the first `pilot.pause()`. Wrapping the queries in a `_poll_until` would mask it cheaply.
