...
```bash
# ~/.config/adk/slack-credentials.sh — chmod 600
...
1. Creates `~/.config/adk/` skeleton (learning/, metadata/, memory/).
...
6. Seeds `~/.config/adk/learning/decisions.jsonl` from `shared/seed-decisions.jsonl` on first install (so `/adk-improve` has evidence from day one).
...
|---|---|
| Symlink skills + agents + commands | Scaffold `~/.config/adk/overrides.yaml` |
...
Edit `~/.config/adk/overrides.yaml` to fill workspaces, repos, and data sources. See [overrides.yaml](../usage/overrides-yaml.md) for the schema.
...
Removes all `adk-*` symlinks, strips the marker block from `CLAUDE.md`, removes `_adk_managed` hook entries. Your `~/.config/adk/` data (overrides + decision log + metadata) is left untouched.
