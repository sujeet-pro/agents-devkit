...
4. **Map to DevKit skills** — match prompt signals to skills:
   - PR URL or "review this PR" → `/review`
   - "bug", "broken", "doesn't work" → `/develop --mode debug`
   - "write ADR", "document" → `/write --type adr`
   - "audit", "security review" → `/audit-security`
   - "migrate", "upgrade" → `/develop --mode migrate`
   - "plan", "design" → `/plan` or `/design`
...
```json
{
  ...
  "action": "review | develop | write | audit | research | plan | design | migrate",
