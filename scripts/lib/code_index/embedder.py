...
from _lib_common import get_logger, write_json, emit_json, die  # noqa: E402
...
# Substrings that indicate the input is too long for the model's context.
# Improvement #8: when ollama returns one of these, retries are pointless —
# the same too-long input will fail the same way three times. Short-circuit
# so we don't burn ~14s of backoff per oversized chunk.
_OVERSIZED_400_HINTS = (
...
def _is_oversized_400(err: Exception) -> bool:
    """Heuristic — match the ollama error body for known length-cap signatures."""
    msg = str(err).lower()
    return any(h in msg for h in _OVERSIZED_400_HINTS)
...
def embed_batch(model: str, texts: list[str], keep_alive: str = "5m",
    ...
    for attempt in range(retries):
        try:
            ...
            last_err = e
            # If the error clearly says "too long", retries are pointless.
            # Surface immediately so the caller can probe per-chunk and skip
            # the offender (improvement #8).
            if _is_oversized_400(e):
                if log:
                    log.info("embed batch: oversized input; skipping retries (saved ~%ds)", 2 * (2 ** retries - 1))
                raise RuntimeError(f"embed input too long: {e}") from e
            wait = 2 * (2 ** attempt)
            if log:
...
def main() -> int:
    ...
    n_batches = 0
    dim: int | None = None
    table = db.open_table(table_name) if (args.mode == "incremental" and table_name in db.list_tables().tables) else None
    started = time.time()
