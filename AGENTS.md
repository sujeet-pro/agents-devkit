...
| "deep PR review" / "thorough review of <PR-URL>" / "check the PR with full code context" | `/adk-pr-review` (heavyweight — owns a worktree, builds embeddings + SCIP, traces feature flags and experiments) |
| "review my PR queue" / "run the PR queue" / "review the list of PRs in `<csv>`" / "batch PR review" / cron-style "go through the open PRs" | `/adk-pr-reviews` (batch over a CSV; skips merged + stable; per-PR locks make N tabs safe) |
