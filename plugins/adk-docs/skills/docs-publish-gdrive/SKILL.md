---
name: docs-publish-gdrive
description: |
  Publish a markdown document to Google Drive as a Google Doc, plain markdown file, or PDF via the workspace Google Drive connector (NO MCP ships with this plugin). Supports folder placement, format selection, and post-publish re-fetch verification. Idempotent via match-by-name-and-parent: same input converges to the same Drive item rather than multiplying duplicates on retry. NEVER changes sharing permissions automatically — sharing is a human action per policy; the skill publishes into the requested folder and inherits that folder's sharing. NEVER moves or deletes existing items. Use when the destination is Google Drive. Do NOT use for Confluence (/adk-docs:docs-publish-confluence), for reading an existing GDoc (use the workspace Google Drive connector directly), or for the authoring step itself (/adk-docs:docs-write).
metadata:
  category: docs
  kind: task
  layer: 5
  modes: [auto, interactive]
  needs_mcp: [google-drive-workspace]
  needs_meta_info: [docs]
argument-hint: "<md-file> [--folder <folder-id>] [--format <gdoc|md|pdf>] [-i]"
---
