...
name: temp-folder
description: |
  Canonical .temp/task-<slug>/ working-artifact layout. Every other adk skill writes through this convention. Use this skill whenever you need to create a new task workspace, look up the path for a particular artifact, or document the layout. The skill wraps the bin/adk-task-slug script for slug generation. Do NOT put final / committed artifacts here — .temp/ is gitignored. Do NOT use this skill to delete or clean .temp/ — that is the operator's manual job (the folder is durable session context).
metadata:
  category: meta
  kind: task
  modes: [auto]
  layer: 0
argument-hint: "<slug-or-prompt> [--scope <path>] [--print-only]"
---
