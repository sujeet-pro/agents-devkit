"""TUI preferences — split ratio and comments filter.
...
Resolution order (highest priority first):
  ...
  3. Hardcoded defaults: ``50`` percent.
...
Backwards-compat: a sidecar that still contains ``"layout"`` or
``"direction"`` keys is silently accepted; those keys are ignored.
"""
...
class LayoutPrefs:
    ...
    def normalised(self) -> "LayoutPrefs":
        ...
        return LayoutPrefs(split_percent=sp, comments_filter=cf)
